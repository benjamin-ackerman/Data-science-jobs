...
```{r plot jobs, eval=FALSE,echo=FALSE,warning=FALSE,message=FALSE,fig.align='center',fig.height=6,fig.width=8}
mapdat = stackjobs %>% 
...
```{r actual map,echo=FALSE,warning=FALSE,message=FALSE,fig.align='center',fig.height=4,fig.width=8.5}
mapdat1 = stackjobs %>% 
  group_by(LocationString) %>% 
  count() %>% 
  inner_join(stackjobs,by="LocationString") %>% 
...
us=ggplot()+borders(database="state")+geom_hex(data=mapdat1, mapping=aes(x=lon,y=lat), bins=12,alpha=.8)+scale_fill_gradient(low = "yellow", high = "red")+theme(
...
europe=ggplot()+borders(database="world",xlim=c(-10,25),ylim=c(40,70))+geom_hex(data=mapdat2, mapping=aes(x=lon,y=lat), bins=12,alpha=.8)+scale_fill_gradient(low = "yellow", high = "red")+theme(
              axis.title.x = element_blank(),
               axis.line = element_blank(),
               axis.text.x=element_blank(),
               axis.ticks.x=element_blank(),
...
cowplot::plot_grid(us,europe, labels = c("USA", "Europe"), align = "v")
```
