...
toptags = stackjobs %>% 
  ...
  arrange(desc(n)) %>% 
  filter(!is.na(tags)) %>% 
  filter(!tags %in% c("statistics","bigdata","javascript"))

tags = ggplot(toptags[1:10,], aes(reorder(tags,n),n)) + geom_col() + coord_flip() + labs(y="Count",x="Tags")+theme(axis.title.x=element_text(size=11),
...
majors = ggplot(topmajors[1:10,], aes(reorder(majors,n),n)) + geom_col() + coord_flip() + labs(y="Count",x="Area of Study")+theme(axis.title.x=element_text(size=11),
               axis.title.y=element_text(size=11),
               axis.text.x = element_text(size=10),
               axis.text.y = element_text(size=10))
...
locations = ggplot(toplocations[1:10,], aes(reorder(locations,n),n)) + geom_col() + coord_flip() + labs(y="Count",x="City")+theme(axis.title.x=element_text(size=11),
               axis.title.y=element_text(size=11),
               axis.text.x = element_text(size=10),
               axis.text.y = element_text(size=10))
...
bottom_row = plot_grid(locations,labels=c("Top 10 Cities"),scale=.9,hjust=-.35,label_size=12)
...
*run t-tests and anova to look at differences in the table below*
...
diffs=diff_analysis %>% 
  group_by(region) %>% 
  mutate(degree = TRUE) %>% 
  select(-bachelors,-masters,-phd) %>% 
  spread(highest_degree,degree) %>% 
  summarise(visa = sum(OffersVisaSponsorship),
            remote = sum(AllowsRemote),
            relocation = sum(OffersRelocation),
            ba_highest = sum(bachelors,na.rm=TRUE),
            ms_highest = sum(masters,na.rm=TRUE),
            phd_highest = sum(phd,na.rm=TRUE),
            n=n())

test = matrix(nrow=ncol(diffs)-1,ncol=2)
for(i in 2:ncol(diffs)){
  percent = t(as.matrix(diffs[,i]))/t(as.matrix(diffs[,ncol(diffs)]))
  test[i-1,] = paste0(t(as.matrix(diffs[,i]))," (",round(percent,3)*100,"%)")  
}
test = test[-nrow(test),]
test = rbind(test[1:3,],c("",""),test[4:6,])
#test = cbind(c("","","","","Bachelors","Masters","PhD"),test[,c(2,1)])
row.names(test)=c("Visa Sponsorship","Allows Remote Work","Offers Relocation","Highest Degree Required:","Bachelors","Masters","PhD")
colnames(test)=c(paste0("Europe (n=",diffs$n[1],")"),paste0("USA (n=",diffs$n[2],")"))

kable(test[,c(2,1)])

# diff_analysis %>% 
#   group_by(region) %>% 
#   summarise(missing = sum(is.na(highest_degree)))
