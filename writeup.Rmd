...
packages = c("devtools","qdapRegex","knitr","dplyr","kableExtra","ggmap","leaflet","stringr","tidyr","tm","SnowballC","wordcloud","RColorBrewer","rebus","grid")
...
map1 <- get_map(location = c(lon = -95, lat = 35),zoom=4,maptype='terrain')

ggmap(map1) + scale_y_continuous(limits=c(20,50)) + geom_point(aes(x = lon, y = lat,size=n),colour="red",alpha=.1, data = mapdat)+theme(legend.position="none")

mapdat$LocationString[which.min(mapdat$lat)]
# library(RgoogleMaps)
  # map=GetMap(zoom=2)
  # PlotOnStaticMap(map)
  
# world <- map_data("world")
# ggplot(world)+geom_point(aes(x = lon, y = lat,size=n),colour="red",alpha=.2, data = mapdat)+theme(legend.position="none")
```
