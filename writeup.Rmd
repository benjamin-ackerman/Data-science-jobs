...
```{r plot ranks, echo=FALSE}
plotRanks <- function(a, b,c,d,e, labels.offset=0.1, arrow.len=0.1)
  {
  ...
  plot(rep(1, len.1), 1:len.1, type='n',# cex=0.8, 
       xlim=c(0, 20), ylim=c(0, max(len.1, len.2,len.3,len.4,len.5)+1),
...
# Print the table
kable(diff_tab, format = "latex", booktabs = T,caption="Differences in Job Listings in the USA vs. Europe") %>%group_rows("Highest Degree Preferred:[note]", 4, 6) %>% 
...
par(mfrow=c(3,1))
plotRanks(tag_trend[,1],tag_trend[,2],tag_trend[,3],tag_trend[,4],tag_trend[,5])
plotRanks(major_trend[,1],major_trend[,2],major_trend[,3],major_trend[,4],major_trend[,5])
plotRanks(city_trend[,1],city_trend[,2],city_trend[,3],city_trend[,4],city_trend[,5])
