...
us=ggplot()+borders(database="state")+geom_hex(data=mapdat1, mapping=aes(x=lon,y=lat), bins=15,alpha=.8)+scale_fill_gradient(low = "yellow", high = "red")+theme(
...
europe=ggplot()+borders(database="world",xlim=c(-10,25),ylim=c(40,70))+geom_hex(data=mapdat2, mapping=aes(x=lon,y=lat), bins=15,alpha=.8)+scale_fill_gradient("# of Jobs",low = "yellow", high = "red")+theme(
               axis.title.x = element_blank(),
               axis.line = element_blank(),
               axis.text.x=element_blank(),
               axis.ticks.x=element_blank(),
...
cowplot::plot_grid(us,europe, labels = c(paste0("USA (n=",nrow(mapdat1),")"), paste0("Europe (n=",nrow(mapdat2),")")), align = "v")
```

```{r,echo=FALSE }
commparison_data = rbind(mapdat1,mapdat2)
```
