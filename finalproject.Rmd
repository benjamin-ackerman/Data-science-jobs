...
```{r datajob listings, echo=FALSE,eval=TRUE,cache=TRUE,warning=FALSE,results='asis'}
...
```{r plot Data Jobs jobs on map, echo=FALSE,message=FALSE,fig.align='center',fig.height=8,fig.width=10,cache=TRUE,warning=FALSE}
...
```{r get zipcodes,eval=FALSE, echo=FALSE,message=FALSE,fig.align='center',fig.height=8,fig.width=10,cache=TRUE,warning=FALSE}
...
m <- leaflet() %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addMarkers(lng=test$lon, lat=test$lat, popup = paste0("<b><a href='",test$link,"'>",test$company," - ",test$position,"</a></b>"))
