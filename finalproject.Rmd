...
I have also begun scraping data from Stack Overflow, by searching for jobs with "Data Scientist" listed. Here's some of the data I've been able to extract:
...
about_job = data.frame(job_type=rep(NA,length(unlist(links))), experience=rep(NA,length(unlist(links))), role=rep(NA,length(unlist(links))), industry=rep(NA,length(unlist(links))),company_size=rep(NA,length(unlist(links))),company_type=rep(NA,length(unlist(links))), link=unlist(links))

for(j in 1:nrow(about_job)){
  listing = readLines(unlist(links)[j])
  search.limit=grep("About this job",listing)[2]

  descriptions<-unlist(rm_between(listing[search.limit:(search.limit+50)],"<span class=\"-key\">",":",extract=TRUE)) 
  ...
  responses<-unlist(rm_between(listing[search.limit:(search.limit+50)],"<span class=\"-value\">","</span>",extract=TRUE))
...
```{r print stackexchange, echo=FALSE, results='asis',cache=TRUE}
kable(about_job[1:10,])
```
