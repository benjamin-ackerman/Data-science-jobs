...
library(devtools);library(qdapRegex);library(knitr);library(dplyr);library(kableExtra);library(ggmap);library(leaflet)
library("tm")
library("SnowballC")
library("wordcloud")
...
kable(about_job[1:10,!names(about_job) %in% c("tags",'role')])
```

There are `r nrow(about_job)` jobs listed on Stack Overflow that come up in my search from `r Sys.Date()`.
...
```{r get zipcodes,eval=FALSE,cache=TRUE, echo=FALSE,message=FALSE,fig.align='center',fig.height=8,fig.width=10,cache=TRUE,warning=FALSE}
...
write.csv(about_job[,names(about_job) != "tags"],file=paste0('/Users/backerman/Documents/JHSPH/3rd Year/Advanced Data Science/stackoverflowjob_',Sys.Date(),".csv"),row.names=FALSE,quote=TRUE)
# Stack Overflow job tags
write.table(unlist(about_job$tags),paste0('/Users/backerman/Documents/JHSPH/3rd Year/Advanced Data Science/stackoverflowjobtags_',Sys.Date(),".txt"),quote=FALSE,row.names=FALSE,col.names=FALSE)
```

Here, I visualize where jobs posted on Stack Overflow are located.  I use the function `geocode` to obtain latitude and longitude coordinates on the cities.  I use code from [this repository](https://github.com/lmullen/mullenMisc/blob/master/R/jitter-latlong.R) to jitter the latitude and longitude, so that multiple points per city could be seen.

```{r plot stack overflow jobs on map, eval=TRUE, echo=FALSE,message=FALSE,fig.align='center',fig.height=8,fig.width=10,cache=TRUE,warning=FALSE}
about_job = read.csv("/Users/backerman/Documents/JHSPH/3rd Year/Advanced Data Science/stackoverflowjob_2017-09-18.csv",header=TRUE)

m <- leaflet() %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addMarkers(lng=about_job$lon, lat=about_job$lat, popup = paste0("<b><a href='",about_job$link,"'>",about_job$company," - ",about_job$position,"</a></b>"))
             #,clusterOptions = markerClusterOptions())
m
```

## 3. Skills that Employers are Looking For
Here is a word cloud of technical skill tags from the Stack Overflow job listings:
...
## Things to do next: 
- Just look at data from Stack Overflow from jobs related to "data"
- Waiting to obtain data dump from David Robinson at Stack Overflow
...
### Analysis Plan:
- Look at trends in programming over time (though jobs only last about 4 weeks on Stack Overflow)
