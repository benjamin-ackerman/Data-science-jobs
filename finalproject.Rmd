...
### Extract data on degree requirements
bachelors_degree <- or("[Bb]achelor","BA" %R% optional("/")%R% "BS", "B" %R% optional('.') %R% "S","B" %R% optional('.') %R% "A")
#table(str_match(newdat$Requirements,bachelors_degree))
#table(str_match(newdat$Description,bachelors_degree))

test<-matrix(str_match(c(newdat$Requirements,newdat$Description),bachelors_degree),ncol=2,byrow = FALSE)
rowSums(test,na.rm = TRUE)

masters_degree <- or("[Mm]" %R% "aster" %R% optional("'") %R% optional("s"),"MSc")
table(str_match(newdat$Requirements,masters_degree))
#table(str_match(newdat$Description,masters_degree))

phd_degree <- or("P" %R% optional('.') %R% "h" %R% optional('.') %R% or("D","d"),"[Dd]octorate")
table(str_match(newdat$Requirements,phd_degree))
#table(str_match(newdat$Description,phd_degree))

newdat$Bachelors = ifelse(rowSums(cbind(as.numeric(str_detect(newdat$Requirements,bachelors_degree)),as.numeric(str_detect(newdat$Description,bachelors_degree))),na.rm = TRUE)==0, 0, 1)
newdat$Masters = ifelse(rowSums(cbind(as.numeric(str_detect(newdat$Requirements,masters_degree)),as.numeric(str_detect(newdat$Description,masters_degree))),na.rm = TRUE)==0, 0, 1)
newdat$PhDs = ifelse(rowSums(cbind(as.numeric(str_detect(newdat$Requirements,phd_degree)),as.numeric(str_detect(newdat$Description,phd_degree))),na.rm = TRUE)==0, 0, 1)
...
test<-lapply(req_nums,get_requirements)

### Try to get skills out of the requirements, match them to the tags on the posts
TT = function(x,i){
  are_tags = any(str_detect(test[[x]],SPC %R% case_insensitive(unique_tags[i])%R%or(SPC,",")))
  sum(are_tags)
}

ntags=sapply(1:length(unique_tags), 
             function(i){
               sum(sapply(1:length(test),function(x){TT(x,i)}))})
moretags = data.frame(tags=unique_tags,n=ntags)

# Look at years of experience
# yrs<- or(capture(optional(DGT) %R% DGT %R% optional(SPC) %R% or("-","to") %R% optional(SPC) %R% optional(DGT) %R% DGT) ,capture(optional(DGT) %R% DGT) %R% optional(SPC) %R% optional(PLUS)) %R% optional(SPC) %R% or("[Yy]ear","[Yy]r") 
# 
# TEST<-cbind(str_match(newdat$Requirements,yrs),str_match(newdat$Description,yrs))
# 
# newdat$link = paste0("http://stackoverflow.com/jobs/",newdat$JobId)
