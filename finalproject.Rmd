...
There are `r nrow(about_job)` unique jobs listed on Stack Overflow that come up in my search from `r Sys.Date()`.

In addition to the manual scraping I have performed, I have also received a database of jobs posted on Stack Overflow over the years from David Robinson, a Data Scientist at Stack Overflow.  Per his request, the data will be kept private.
...
Here, I visualize where jobs posted on Stack Overflow are located *using the data I scraped manually*.  I use the function `geocode` to obtain latitude and longitude coordinates on the cities.  I use code from [this repository](https://github.com/lmullen/mullenMisc/blob/master/R/jitter-latlong.R) to jitter the latitude and longitude, so that multiple points per city can be seen.
...
Next, I visualize the Stack Overflow jobs using data provided from David Robinson.  Markers are color coded by the year that they were posted.
```{r load stack overflow jobs and work with it, message=FALSE,fig.align='center',fig.height=8,fig.width=10,warning=FALSE,echo=FALSE}
# Read in edited data
...
colors<-c("lightred","red","orange","lightgreen","green","lightblue","blue","purple")
