...
stem = c(stem,"applied math","CS","operation research","Computational Physics")
...
majors_wanted = lapply(majors_wanted,function(x){str_replace_all(x,"CS","Computer Science")})
majors_wanted = lapply(majors_wanted,function(x){str_replace_all(x,"applied math","Applied Mathematics")})
majors_wanted = lapply(majors_wanted,function(x){str_replace_all(x,"operation research","Operations Research")})

newdat$majors = NA
newdat$majors[req_nums] = majors_wanted
...
pop_majors = data.frame(sort(table(unlist(stackjobs$majors)),decreasing=TRUE)[1:10])
names(pop_majors) = c("Major","n")
kable(pop_majors,caption="10 most common majors listed in job postings")
```
