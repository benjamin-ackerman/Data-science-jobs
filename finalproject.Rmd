...
# Function to clean requirements data
...
bachelors<-lapply(reqs, function(x){x[str_detect(x,bachelors_degree) & !str_detect(x,no_bachelor)]})
#bachelors[lapply(bachelors,length)>0]

newdat$bachelors=ifelse(is.na(newdat$Requirements),NA, 
                      ifelse(as.numeric(row.names(newdat)) %in% req_nums[lapply(bachelors,length)>0],1,0))

# Indication of masters degrees:
masters_degree <- or("[Mm]" %R% "aster" %R% optional("'") %R% "s","MSc","M"%R%optional(DOT)%R%"S"%R%optional(DOT))

# get rid of these from the regular expression:
...
masters<-lapply(reqs, function(x){x[str_detect(x,masters_degree)& !str_detect(x,no_masters)]})
#masters[lapply(masters,length)>0]

newdat$masters=ifelse(is.na(newdat$Requirements),NA, 
                      ifelse(as.numeric(row.names(newdat)) %in% req_nums[lapply(masters,length)>0],1,0))

phd_degree <- or("P" %R% optional('.') %R% "h" %R% optional('.') %R% or("D","d"),"[Dd]octorate")

phds<-lapply(reqs, function(x){x[str_detect(x,phd_degree)]})
#phds[lapply(phds,length)>0]

newdat$phd=ifelse(is.na(newdat$Requirements),NA, 
                      ifelse(as.numeric(row.names(newdat)) %in% req_nums[lapply(phds,length)>0],1,0))

#table(newdat$bachelors,(newdat$masters==1 | newdat$phd == 1)) #More jobs list Masters or Phd without Bachelors


## Find STEM majors:
test<-readLines("http://stemdegreelist.com/stem-degree-list/")
stem<-as.character(na.omit(unlist(rm_between(test,"<li>","</li>",extract=TRUE))))
stem = c(stem,"applied math","CS")

# stem<-unique(as.vector(str_split(stem,or(" and ","/",", "),simplify = TRUE))) %>% 
#   str_replace_all(DGT,"") %>% 
#   str_replace_all("&amp;","&") %>% 
#   str_replace_all(or("\\(","\\)"),"") 
# stem = stem[str_detect(stem,exactly(""))==FALSE]

# Find all of the lines in the requirements that mention some degree requirement:
# mentions_degree<-lapply(reqs, function(x){x[str_detect(x,phd_degree) | (str_detect(x,masters_degree)& !str_detect(x,no_masters)) | (str_detect(x,bachelors_degree) & !str_detect(x,no_bachelor))]})
# 
# mentions_degree<-mentions_degree[lapply(mentions_degree,length)>0]

# Extract all of the requested majors based on the STEM
majors_wanted = list()
for(i in 1:length(reqs)){
  majors_wanted[[i]]=unique(names(which(sapply(stem, function(x){
    any(str_detect(reqs[[i]],"\\b"%R%case_insensitive(x)%R%"\\b"))})==TRUE)))
}

for(i in which(lapply(majors_wanted,length)>0)){
    if(sum(sapply(mathstrings,function(x){str_detect(majors_wanted[[i]],case_insensitive(exactly(x)))}))==2){
majors_wanted[[i]]=majors_wanted[[i]][-which(str_detect(majors_wanted[[i]],case_insensitive(exactly("Mathematics"))))]}
}

majors_wanted[lapply(majors_wanted,length)>0]

sort(table(unlist(majors_wanted)),decreasing=TRUE)[1:20]

newdat$majors = ifelse(is.na(newdat$Requirements),NA,
                       ifelse(as.numeric(row.names(newdat))%in%req_nums[lapply(majors_wanted,length)>0],
                              majors_wanted[lapply(majors_wanted,length)>0], NA))
test<-ifelse(is.na(newdat$Requirements),NA,
                       ifelse(as.numeric(row.names(newdat))%in%req_nums[lapply(majors_wanted,length)>0],
                              majors_wanted[lapply(majors_wanted,length)>0], NA))
...
jobs_per_year<-newdat %>% 
  group_by(year) %>% 
  count()

degree_demand <- newdat %>% 
  group_by(year) %>%  
  summarise(Bachelor = sum(Bachelors),
            Master = sum(Masters),
            PhD = sum(PhDs)) %>% 
  ...
  summarise(Bachelor = Bachelor/n,
            Master = Master/n,
            PhD = PhD/n) %>% 
  gather(key=degree,value=prop,-year)


degree_demand <- newdat %>% 
  filter(year >2011) %>% 
  group_by(year,degree_latest2) %>%  
  count() #%>% 
  #inner_join(jobs_per_year,by='year') %>% 
  #filter(!is.na(degree_latest2)) %>% 
  #mutate(prop = n.x/n.y)
#p <- ggplot(na.omit(degree_demand),aes(x=year,y=prop,group=degree_latest2,colour=degree_latest2))
p <- ggplot(na.omit(degree_demand),aes(x=year,y=n,group=degree_latest2,colour=degree_latest2))
p + geom_line()


p = ggplot(degree_demand,aes(x=year,y=prop*100,group=degree,colour=degree))
p + geom_line()
```
